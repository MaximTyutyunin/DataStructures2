...
public class A1Q1cosc2007 {
    ...
    public static void main(String[] args) {
        ...
        int[][] maze = null;
/**
 * 1)make scanner read from buffer
 * 2)String[] line = sf.nextLine().trim().split(" "); creates array of
 *   strings that contains parameters for the maze.
 *      .trim() cuts all extra spaces at the end of a line,
 *      .split(" ") splits this string around matches of the given parameter
 * 3)use same way to input all numbers from file into array 'maze"
 *
 * */
        try {
            ...
            if (sf.hasNextLine()) {
                ...
                System.out.println("Array size was not provided in file");
                System.exit(1);
            }
...
/**
 * Assignment doesnt specify what the input should look like so i made this.
 * Following code is quite easy to read so i desided to stick to it.
 * */
        int colStart = 0;
        int rowStart = 0;
        int colEnd = 0;
        ...
        String colEndS = sc.nextLine();

        System.out.println("yEnd:");

        String rowEndS = sc.nextLine();

        if(inputCheck(colStartS, rowStartS, colEndS, rowEndS) == 1) {// check if there are any letters instead if digits

            colStart = Integer.parseInt(colStartS);
            rowStart = Integer.parseInt(rowStartS);
            colEnd = Integer.parseInt(colEndS);
            rowEnd = Integer.parseInt(rowEndS);
        }else{
            return;
        }
    ...
    public static void searchMaze(int[][] maze, int length, int height, int rowStart, int colStart, int rowDest, int colDest, int stepCounter) {
         ...
         * deleting all unnecessary steps and
    ...
    public static int inputCheck(String colStart, String rowStart, String colEnd, String rowEnd) {
        /**
        ...
        String originalStr = "( " + colStart + " , " + rowStart + " )" + " ( " + colEnd + " , " + rowEnd + " )";
        String strNumbersOnly = colStart + rowStart + colEnd + rowEnd;
        Pattern pattern1 = Pattern.compile("[-]");//this and the following lines fetch digits from string
        strNumbersOnly = pattern1.matcher(strNumbersOnly).replaceAll("");


        Pattern pattern2 = Pattern.compile("[^0-9]");//this and the following lines fetch digits from string
        String numberOnly = pattern2.matcher(strNumbersOnly).replaceAll("");

        int numberOfInvalidCoord = 0;
        if (numberOnly.length() < strNumbersOnly.length()) {
            numberOfInvalidCoord = strNumbersOnly.length() - numberOnly.length();

            System.out.println(numberOfInvalidCoord + " invalid Character(s): " + originalStr);
            return 0;
        }
        return 1;
    }
...
/**
 * 1) check if numbers a too large than, if they are too small
 * 2)check if destination is 0 which is wrong
 * 3)than goes code that was probably not expected but i for some reason
 *   decided to make my life harder and probably loose marks.
 *   I check if destination point is surrounded with "0"
 *   There are special cases when destination is at borders of mase so i didnt came up with anything better than
 *   just create "if" statements for every case
 *
 * */
        if (colDest >= maze.length || rowDest >= maze.length) {
            System.out.println("Invalid End with a number " +
                    "that’s too large ( " + colDest + "," + rowDest + " )");
            return 0;

        }
        if (colDest < 0 || rowDest < 0) {
            System.out.println("Invalid End with a number" +
                    " that’s too small ( " + colDest + "," + rowDest + " )");
            return 0;
